...
AddEventHandler('esx_policeinteractions:getarrested', function(playerheading, playercoords, playerlocation)
	...
	lib.notify({
		title = 'Poliisi',
		description = 'Sinut on raudoitettu',
		position = "top"
	})
     SetCurrentPedWeapon(playerPed, `WEAPON_UNARMED`, true)  
...
AddEventHandler('esx_policeinteractions:sech', function()
	...
	lib.notify({
		title = 'Poliisi',
		description = 'Taskujasi Tutkitaan',
		position = 'top',
		style = {
...
AddEventHandler('esx_policeinteractions:putInVehicle', function()
	...
	lib.notify({
		title = 'Poliisi',
		description = 'Sinut laitettiin ajoneuvoon',
		position = 'top',
		style = {
			backgroundColor = '#141517',
			color = '#C1C2C5',
...
RegisterNetEvent('impound', function(...)
	--local vehicleName = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
	DeleteVehicle()
	print('Event fired')
  end)
...
exports.ox_target:addGlobalVehicle(impound)